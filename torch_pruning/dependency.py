...
class DependencyGraph(object):
    ...
    def build_dependency(
        ...
        self.backward_pruning = backward_pruning
    ...
    def get_all_groups_in_order(self, ignored_layers=[], group_idxs=None, root_module_types=(ops.TORCH_CONV, ops.TORCH_LINEAR)):
        visited_layers = []
        ignored_layers = ignored_layers+self.IGNORED_LAYERS

        modules = list(self.model.named_modules())
        if self.backward_pruning:
            modules.reverse()
        ...
        for _, m in modules:
            ...
            if prunable_group:
                if group_idxs == None or idx in group_idxs:
                    yield group
                idx += 1

    def get_all_groups(self, ignored_layers=[], root_module_types=(ops.TORCH_CONV, ops.TORCH_LINEAR), layer_idx=None):
        visited_layers = []
        ignored_layers = ignored_layers+self.IGNORED_LAYERS
        li = list(self.module2node.keys())

        if not self.backward_pruning:
            li.reverse()
        ...
        for m in li:
            ...
            if prunable_group:
                ...
                if layer_idx != None and not (idx == layer_idx or (isinstance(layer_idx, list) and idx in layer_idx)):
                    idx += 1
                    continue
                idx += 1
                yield group
