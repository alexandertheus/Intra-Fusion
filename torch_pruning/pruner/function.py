...
class BasePruningFunc(ABC):
    ...
    def check(self, layer, idxs, to_output):
        if self.TARGET_MODULES is not None:
            assert isinstance(
                layer, self.TARGET_MODULES
            ), "Mismatched pruner {} and module {}".format(self.__str__, layer)
        if to_output:
        ...
        if prunable_channels is not None:
            assert all(
                idx < prunable_channels and idx >= 0 for idx in idxs
            ), "All pruning indices should fall into [{}, {})".format(
                0, prunable_channels
            )
    ...
    def _prune_parameter_and_grad(self, weight, keep_idxs, pruning_dim, ot_map=None, dimensionality_preserving: bool = False):
        if ot_map != None:
            ...
            pruned_weight = torch.nn.Parameter(torch.einsum(einsum_str, ot_map, weight))
            ...
            if weight.grad is not None:
                ...
                pruned_weight.grad = torch.nn.Parameter(torch.einsum(einsum_str, ot_map, weight.grad))
...
class LayernormPruner(BasePruningFunc):
    ...
    def prune_out_channels(self, layer: nn.Module, idxs: Sequence[int]) -> nn.Module:
        ...
        if layer.elementwise_affine:
            layer.weight = self._prune_parameter_and_grad(
                layer.weight, keep_idxs, pruning_dim
            )
            layer.bias = self._prune_parameter_and_grad(
                layer.bias, keep_idxs, pruning_dim
            )
        if pruning_dim != -1:
...
class LSTMPruner(BasePruningFunc):
    ...
    def prune_out_channels(self, layer: nn.LSTM, idxs: list) -> nn.Module:
        ...
        expanded_keep_idxs = torch.cat(
            [keep_idxs + i * num_features for i in range(4)], dim=0
        )
        ...
        for pf in postfix:
            setattr(
                layer,
                "weight_hh_l0" + pf,
                self._prune_parameter_and_grad(
...
class ParameterPruner(BasePruningFunc):
    ...
    def prune_out_channels(self, tensor, idxs: list) -> nn.Module:
        ...
        keep_idxs.sort()
        pruned_parameter = self._prune_parameter_and_grad(
            tensor, keep_idxs, self.pruning_dim
        )
...
class MultiheadAttentionPruner(BasePruningFunc):
    ...
    def prune_out_channels(self, layer, idxs: list) -> nn.Module:
        ...
        if layer.q_proj_weight is not None:
            layer.q_proj_weight = self._prune_parameter_and_grad(
                layer.q_proj_weight, keep_idxs, 0
            )
        if layer.k_proj_weight is not None:
            layer.k_proj_weight = self._prune_parameter_and_grad(
                layer.k_proj_weight, keep_idxs, 0
            )
        if layer.v_proj_weight is not None:
            layer.v_proj_weight = self._prune_parameter_and_grad(
                layer.v_proj_weight, keep_idxs, 0
            )
        ...
        if layer.in_proj_weight is not None:
            layer.in_proj_weight = self._prune_parameter_and_grad(
                layer.in_proj_weight, keep_idxs_3x_repeated, 0
            )
            layer.in_proj_weight = self._prune_parameter_and_grad(
                layer.in_proj_weight, keep_idxs, 1
            )
...
prune_depthwise_conv_out_channels = PrunerBox[
    ops.OPTYPE.DEPTHWISE_CONV
].prune_out_channels
prune_depthwise_conv_in_channels = PrunerBox[
    ops.OPTYPE.DEPTHWISE_CONV
].prune_in_channels
