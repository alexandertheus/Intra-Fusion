...
class VGG(nn.Module):
    def __init__(
        self, vgg_name, num_classes=10, batch_norm=True, bias=True, relu_inplace=True
    ):
        super(VGG, self).__init__()
        self.batch_norm = batch_norm
        self.bias = bias
        self.features = self._make_layers(
            cfg[vgg_name], relu_inplace=relu_inplace)
        ...
        for x in cfg:
            if x == "M":
                ...
                if self.batch_norm:
                    layers += [
                        nn.Conv2d(
                            in_channels, x, kernel_size=3, padding=1, bias=self.bias
                        ),
                        nn.BatchNorm2d(x),
